...
const BatteryIcon = ({ isValueVisible, isForcedDarkMode = false }) => {
  ...
  return (
    <>
      {isValueVisible && (
        <span className="text-xs mr-1">{`${(batteryLevel * 100).toFixed()}%`}</span>
      )}
      <div className="relative">
        ...
        {isCharging && <BsLightningChargeFill className={classNames(lightningChargeClasses)} />}
      </div>
    </>
  );
};
